...
// const page = ref<number>(props.info.page?.current || 1);
// const perPage = ref<number>(props.info.page?.perPage || 20);
/**
...
function setPage(newPage: number) {
  ...
  emit('change', newPage, props.info.page.perPage);
}
...
<template>
  <nav v-if="info.page" class="pagination is-small" role="navigation">
    ...
    <ul class="pagination-list">
      <li v-for="n in info.page.totalPages" :key="n">
        <a @click.prevent="setPage(n)" class="pagination-link" :class="{ 'is-current': n === info.page.current }">{{ n }}</a>
      </li>
    </ul>
  </nav>
